<template>
    <div @click.self="goToLagerPage" class="bg-white w-screen h-screen flex flex-col justify-center items-center relative">
        ...
        <div class="bg-[#F1F7FF] w-auto h-auto rounded-xl flex flex-col justify-center p-5 items-right relative">
            ...
            <h1 class="font-semibold text-[24px] text-[#00214B] mb-2">Opdater stock</h1>

            <div class="mt-2 indent-[15px] h-[50px] w-[400px] bg-white text-[#00214B] opacity-40 rounded-lg text-left focus:outline-4 focus:outline outline-offset-4 outline-[#0097ff] font-poppins font-semibold">
...
export default {
    ...
    methods: {
        ...
        async updateStock(){
            ...
            try {
                    const ref = doc(db, "stock", this.$store.state.lagerUdInd[this.$store.state.TempIndex].id)
                    await updateDoc(ref, {
                        date: new Date(this.dateValue),
                        update: Number(this.correctionValue)
                    });

                console.log("Updated " + this.$store.state.lagerUdInd[this.$store.state.TempIndex].id + " from: stock");
                this.goToLagerPage()

                } catch (error) {
                console.error("ERROR updating " + this.$store.state.lagerUdInd[this.$store.state.TempIndex].id + " from: stock " + error);
            }
        }
    },
