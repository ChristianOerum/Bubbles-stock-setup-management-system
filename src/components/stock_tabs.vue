<template>
    ...
    <div v-for="(item, index) in this.$store.state.lagerUdInd" v-bind:key="index">
        ...
        <div v-if="(item.Update > 0) == false" class="bg-gradient-to-r from-[#FFFFFF] to-[#FFD4D4] w-[100%] h-[60px] relative grid items-center p-[18px] text-[#00214B] text-[15px] font-semibold" style="grid-template-columns: 3fr 1fr 1fr 40px">
            ...
            <div class="col-start-4 col-span-1 opacity-50">
                ...
                <fa style="cursor: pointer" @click="deleteStock($event, index)" icon="trash-can" class="w-auto h-[16px] ml-2"/>
            </div>

            <div class="bg-[#3A567A] opacity-5 bottom-[0px] w-[100%] h-[4px] absolute"></div>
        </div>

        <div v-else class="bg-gradient-to-r from-[#FFFFFF] to-[#CBF6C3] w-[100%] h-[60px] relative grid items-center p-[18px] text-[#00214B] text-[15px] font-semibold" style="grid-template-columns: 3fr 1fr 1fr 40px">
            <h1 class="col-start-1 col-span-1">{{ item.Produktnavn }}</h1>
